...
import { AttachmentPreview, isPdf } from '@/components/feed/AttachmentPreview';
...
export function LectureCard({ post }: { post: Post }) {
  const [lightboxOpen, setLightboxOpen] = useState(false);
  const canFocus = post.attachmentType === 'lecture' && !!post.attachmentUrl && isPdf(post.attachmentUrl, post.attachmentOriginalName);

  return (
    <div className="rounded-2xl border border-border bg-surface p-4">
      <div className="flex items-start gap-3">
        <Avatar src={assetUrl(post.author?.photoUrl)} name={post.author?.name ?? 'مستخدم محذوف'} size="sm" />
        <div className="min-w-0 flex-1">
          <p className="truncate text-sm font-semibold text-foreground">{post.caption || 'بدون عنوان'}</p>
      ...
      <div className="mt-3 flex flex-wrap gap-1.5">
        {post.courseCode && (
          <span className="rounded-full bg-accent/10 px-2 py-0.5 text-[11px] font-medium text-accent">{post.courseCode}</span>
        )}
        {post.department && (
      ...
      {canFocus && post.attachmentUrl && (
        <LecturePdfLightbox
          ...
          title={post.attachmentOriginalName ?? post.caption ?? 'معاينة PDF'}
        />
      )}
    </div>
  );

...
import { FileText, LayoutGrid, List, Plus, Search, Video } from 'lucide-react';
...
export function LecturesBrowser({
  ...
  const sortedPosts = useMemo(() => {
    const copy = [...posts];
    copy.sort((a, b) => (sort === 'newest' ? b.createdAt.localeCompare(a.createdAt) : a.createdAt.localeCompare(b.createdAt)));
    return copy;
  }, [posts, sort]);

  const specializationOptions = department ? SPECIALIZATIONS_BY_DEPARTMENT[department] : [];
  const academicYearOptions = department ? getAcademicYearsForDepartment(department) : ACADEMIC_YEARS;

  function buildQuery(targetPage: number) {
    const params = new URLSearchParams({ type: attachmentType, page: String(targetPage), limit: String(PAGE_SIZE) });
  ...
  }, [attachmentType, department, academicYear, specialization, courseCode, query]);

  async function loadMore() {
    setLoadingMore(true);
    const nextPage = page + 1;
    const data = await api.get<Post[]>(`/posts/lectures?${buildQuery(nextPage)}`);
  ...
  return (
    <div className="mx-auto w-full max-w-4xl space-y-4 px-4 py-6">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <select value={academicYear} onChange={(e) => setAcademicYear(e.target.value as AcademicYear | '')} className={SELECT_CLASS}>
        ...
        >
          <option value="">{department ? 'كل التخصصات' : 'التخصص'}</option>
        ...
        />
        <select value={sort} onChange={(e) => setSort(e.target.value as 'newest' | 'oldest')} className={SELECT_CLASS}>
        ...
        <div className="flex items-center gap-0.5 rounded-lg border border-border bg-surface-2 p-0.5">
          <button
            ...
            title="عرض قائمة"
            className={cn('rounded-md p-1.5 transition-colors', view === 'list' ? 'bg-surface text-accent shadow-soft' : 'text-muted-foreground hover:text-foreground')}
          >
          ...
          <button
            ...
            title="عرض شبكي"
            className={cn('rounded-md p-1.5 transition-colors', view === 'grid' ? 'bg-surface text-accent shadow-soft' : 'text-muted-foreground hover:text-foreground')}
          >
        ...
        <div className="space-y-3">
          <div className={cn(view === 'grid' ? 'grid grid-cols-1 gap-3 sm:grid-cols-2' : 'space-y-3')}>
            {sortedPosts.map((post) => (
              <LectureCard key={post._id} post={post} />
            ))}
          </div>
          {hasMore && (

    ...
    <div className="flex h-full flex-col">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto scrollbar-thin">
        {loading ? (
          ...
          <div className="mx-4 mt-4 flex flex-col items-center gap-3 rounded-2xl border border-dashed border-border px-6 py-14 text-center">
            <div className="flex h-12 w-12 items-center justify-center rounded-full bg-surface-2/70">
              <Users className="h-5 w-5 text-muted-foreground" />
            </div>
            <p className="text-sm text-muted-foreground">لا توجد مجموعات بعد. أنشئ واحدة أو انضم برمز دعوة.</p>
          </div>
        ) : (
          groups.map((group) => {
            ...
            return (
              <Link
                ...
                href={href}
                className={cn(
                  'flex items-center gap-3 border-b border-border px-4 py-3.5 transition-colors hover:bg-surface-2',

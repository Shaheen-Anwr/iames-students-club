  ...
  async *sendMessageStream(
    ...
    if (usedToday >= this.dailyMessageQuota) {
      ...
      yield {
        type: 'error',
        message:
          `لقد بلغت الحد الأقصى اليومي للأسئلة مع رافد (${this.dailyMessageQuota} سؤالًا). ` +
    ...
    openAiMessages.push({
      ...
      content:
        ...
        (assistantName
          ...
          : 'اسمك الافتراضي «رافد» — قدّم نفسك به إن سُئلت من أنت. وإن ناسب السياق في بداية محادثة جديدة، اسأل الطالب بلطف عن اسم آخر يحب أن يناديك به.'),
    });
